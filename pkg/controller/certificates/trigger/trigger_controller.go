...
func shouldBackoffReissuingOnFailure(log logr.Logger, c clock.Clock, crt *cmapi.Certificate, nextCR *cmapi.CertificateRequest, initialDelay time.Duration, maxDelay time.Duration) (bool, time.Duration) {
	...
	if crt.Status.FailedIssuanceAttempts != nil {
		// math.Pow(2, >62) overflows time.Duration (int64), wrapping to
		// negative. Prevent this by capping at maxDelay for extreme attempt
		// counts that can never be reached through normal exponential growth.
		if *crt.Status.FailedIssuanceAttempts > 62 {
			delay = maxDelay
		} else {
			delay = initialDelay * time.Duration(math.Pow(2, float64(*crt.Status.FailedIssuanceAttempts-1)))
		}
	}

	// Cap delay at the configured maximum backoff duration.
	// This replaces the old stopIncreaseBackoff/maxDelay pair.
	if delay > maxDelay {
		delay = maxDelay
	}
